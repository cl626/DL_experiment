...
#            nn.Dropout(),
    ...
    def valid_forward(self,x):
        x = self.features(x)
        x = x.view(x.size(0), -1)
#        x = self.classifier[1](self.classifier[0](x))*0.5
        x = self.classifier[1](self.classifier[0](x))
#        x = self.classifier[4](self.classifier[3](x))*0.5
        x = self.classifier[3](self.classifier[2](x))
    ...
    if(epoch%10==9):
        ...
        with torch.no_grad():   #切断上下文求导计算
            for data in valid_loader:
                ...
                total+=labels.size(0)
                correct+=(predicted==labels).sum().item()        
        print('Accurac of the network on the 10000 valid images: %d %%'%(100*correct/total))
...
plt.xlabel('epoch')
plt.title('vgg2 average loss,acc-epoch')
plt.legend()
plt.savefig('VGG2.png')